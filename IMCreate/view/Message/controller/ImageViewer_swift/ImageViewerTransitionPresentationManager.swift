...
protocol ImageViewerTransitionViewControllerConvertible {
    ...
    // The source view
    var sourceView: MediaView? { get }
    
    // The final view
    var targetView: MediaView? { get }
    ...
    private func presentAnimation(
        ...
        completed: @escaping((Bool) -> Void)) {
            ...
            UIView.animate(withDuration: duration, animations: {
                if let image = transitionVC.targetView?.image, !transitionVC.fullscreen  {
                    dummyImageView.frame = .init(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.width * image.size.height / image.size.width)
                }
                else {
